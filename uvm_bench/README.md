...
**`chi_smoke_test`** (default) mirrors Cocotb **single-beat** traffic; **`chi_burst_test`** mirrors the **multi-beat** integration choreography (3-beat write, 4-beat read) aligned with **`integration/test_integration.py`**.
...
| `uvm/chi_tb_pkg.sv` | UVM package: driver, monitor, scoreboard, `chi_smoke_seq` / `chi_smoke_test`, `chi_burst_smoke_seq` / `chi_burst_test`. |
...
# or: +UVM_TESTNAME=chi_burst_test
```
...
Burst tests use **`#(8us)`** drain in **`chi_burst_test::run_phase`**; smoke uses **`#(5us)`**.
