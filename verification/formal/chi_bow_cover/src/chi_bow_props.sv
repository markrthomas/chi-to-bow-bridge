...
module chi_bow_props #(
    ...
    // Input constraints
    // ----------------------------------------------------------------
    // Limit txnid to {0,1,2,3}: only 4 bits of pending_txn ever live.
    always @(*) assume (chi_req_txnid[7:2] == 6'd0);
    ...
    // RSP_HDR txnid[121:114] top 6 bits = 0 → txnid ∈ {0..3}.
    always @(*) begin
        if (bow_rx_data[127:124] == PKT_TYPE_RSP_HDR)
            assume (bow_rx_data[121:116] == 6'd0);
    end

    // RSP_DATA txnid[123:116] top 6 bits = 0 → txnid ∈ {0..3}.
    always @(*) begin
        if (bow_rx_data[127:124] == PKT_TYPE_RSP_DATA)
            assume (bow_rx_data[123:118] == 6'd0);
    end
