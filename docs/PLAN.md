...
| Documentation | `docs/design_spec.md`, `docs/integration.md`, **`docs/PLAN.md`**; PDFs via `docs/Makefile`; root **`make docs`** also builds **`uvm_bench/README.pdf`**, **`uvm_bench/UVM_QUICKREF.pdf`**, **`uvm_bench/UVM_ONBOARDING.pdf`**, and **`vlate_bench/README.pdf`** (see **[`README.md`](../README.md)** for the authoritative list and commands) |
...
| Illegal REQ opcodes increment `err_illegal_req_hdr` + `err_pulse` (READ_RESP / WRITE_ACK on REQ) | Integration | - | Implemented - `test_integration_illegal_chi_req_opcodes_increment_err_counter` | Implemented - `make run UVM_TEST=chi_illegal_req_test` (illegal-only parity); stitched smoke+burst+inject+illegal — `make run UVM_TEST=chi_full_integration_test` | Implemented — `tb_main.cpp`: `inject_unknown_txn_rsp_hdr`, settle, double `drive_illegal_req_phase` |

### Integration BoW inject mux scenarios

| Scenario | Topology | Block Cocotb `test/` | Integration Cocotb `integration/` | UVM `uvm_bench/` | Verilator `vlate_bench/` |
|----------|----------|----------------------|-----------------------------------|------------------|---------------------------|
| Unknown txnid BoW `RSP_HDR` bumps `err_unknown_txn_rsp_hdr` | Integration | - | Implemented - `test_integration_unknown_txnid_bow_rsp_hdr_via_inj` | Implemented - `make run UVM_TEST=chi_unknown_txn_inj_test` ; stitched — **`chi_full_integration_test`** | Implemented - `inject_unknown_txn_rsp_hdr` |
...
- **README versus tooling** - The root **[`README.md`](../README.md)** now documents `uvm_bench/` and `vlate_bench/`, the full **`make docs`** PDF outputs (spec/integration/plan PDFs under `docs/` plus **`uvm_bench/README.pdf`**, **`uvm_bench/UVM_QUICKREF.pdf`**, **`uvm_bench/UVM_ONBOARDING.pdf`**, and **`vlate_bench/README.pdf`**), optional prerequisites (VCS / Verilator), a verification-environment summary table, subdirectory commands, and links to this **`PLAN.md`** for backlog context.
...
- **UVM functional + optional VCS structural coverage** - **`chi_integration_cov`** (**`uvm_bench/uvm/chi_tb_cov.svh`**) samples REQ/RSP handshakes, completed **`bow_inj_*`** beats (golden unknown **`RSP_HDR`** tuple), and **`err_pulse`** snapshots of **`err_illegal_req_hdr`** / **`err_unknown_txn_rsp_hdr`**; **`report_phase`** prints four **`[COV]`** **`get_coverage()`** metrics (**`uvm_bench/README.md`** documents bins/crosses). **`make -C uvm_bench coverage`** adds Synopsys **`-cm`** (**`uvm_cov.vdb`**); **`make -C uvm_bench cov-report`** runs **URG** when installed.
